...
void mainloop(Display *xdisplay, EGLDisplay display, EGLSurface surface)
{
	...
	const char* fshader =
		...
		"void main() {\n"
            "vec4 yuyv = texture(tex_yuyv, Flag_uv);\n"
            "vec2 yy = yuyv.xz;\n"
            "vec2 cbcr = yuyv.yw;\n"

            "vec2 tex_size = textureSize(tex_yuyv.x, tex_yuyv.y, tex_yuyv.z, 0);\n"
            "float factor = tex_size.x/rt_width;\n"
            "float cur_y = (Flag_uv.x * factor) < 0.5f ? yy.x : yy.y;\n"
            ...
            "outFragmentColor = texture2D( rgb, 1.f );\n"
		"}\n";
    ...
    GLint id_rt_width = glGetUniformLocation(program, "rt_width");
    ...
    glTexImage2D ( GL_TEXTURE_2D, 0, GL_RGB, TEXWIDTH, TEXHEIGHT, 0, GL_RGB, GL_UNSIGNED_BYTE, texture_yuv);
